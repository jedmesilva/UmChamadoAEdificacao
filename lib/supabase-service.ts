...
export const authService = {
  ...
  async getUserByEmail(email: string): Promise<AccountUser | null> {
    try {
      // Primeiro tenta usar API admin para listar usuários e filtrar pelo email
      // Este método não depende do método específico getUserByEmail que pode não estar disponível
      const { data: authListData, error: authListError } = await supabaseAdmin.auth.admin.listUsers();
      
      if (!authListError && authListData && authListData.users) {
        // Filtra o usuário pelo email (case insensitive)
        const authUser = authListData.users.find(
          u => u.email && u.email.toLowerCase() === email.toLowerCase()
        );
        
        if (authUser) {
          // Encontrou o usuário na autenticação, agora busca o perfil na tabela account_user
          const { data: accountUser, error: accountError } = await supabaseClient
            .from('account_user')
            .select('*')
        ...
        console.error('Erro ao listar usuários na autenticação:', authListError);
      }
      
      // Fallback: busca apenas na tabela account_user usando o email
      const { data, error } = await supabaseClient
        .from('account_user')
        .select('*')
        .eq('email', email)
...
export const subscriptionService = {
  ...
  async checkUserExists(email: string): Promise<boolean> {
    try {
      // Usa o método de listar usuários para verificar se o email existe
      const { data: authListData, error: authListError } = await supabaseAdmin.auth.admin.listUsers();
      
      if (!authListError && authListData && authListData.users) {
        // Filtra o usuário pelo email (case insensitive)
        const userExists = authListData.users.some(
          u => u.email && u.email.toLowerCase() === email.toLowerCase()
        );
      ...
      // Se não encontrou na auth, verifica na tabela account_user
      const { data: accountUser, error: accountError } = await supabaseAdmin
        .from('account_user')
        .select('*')
        .eq('email', email)
