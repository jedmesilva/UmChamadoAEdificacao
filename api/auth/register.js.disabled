...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Criar o perfil do usuário diretamente em account_user
    try {
      console.log(`Criando novo account_user para ${email} com ID ${authUser.id}`);
      
      const { data: newAccountUser, error: insertError } = await supabase
        .from('account_user')
        .insert({
          id: authUser.id,
          user_id: authUser.id,
          email: email,
          name: name,
          status: 'is_complit', // Definir como is_complit já que estamos criando com nome
          // O Supabase automaticamente adiciona timestamps
        })
        .select()
        .single();
        
      if (insertError) {
        console.error(`Erro ao criar perfil do usuário: ${JSON.stringify(insertError)}`);
        console.log(`Usuário registrado apenas na auth.users, mas perfil não pôde ser criado.`);
        ...
        return res.status(201).json({
          ...
          name: name,
          needsProfile: true, // Indica que o perfil ainda precisa ser criado
      ...
      console.log(`Detalhes conta: ${JSON.stringify(newAccountUser)}`);
      ...
      // Mesmo com erro, retornamos sucesso pois o usuário foi criado no auth
      return res.status(201).json({
        message: "Usuário criado com sucesso",
        userId: authUser.id, 
        email: authUser.email,
