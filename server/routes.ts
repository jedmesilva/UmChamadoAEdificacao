...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/cartas/registrar-leitura", async (req, res, next) => {
    ...
    try {
      ...
      try {
        // Buscar a carta usando tanto o id quanto o id_sumary_carta
        const { data: carta, error } = await supabaseClient
          .from('cartas_um_chamado_a_edificacao')
          .select('*')
          .or(`id.eq.${recebido.cartaId},id_sumary_carta.eq.${recebido.cartaId}`)
          .single();
